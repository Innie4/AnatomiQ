  ...
  if (!mounted) {
    return (
      <div className="glass-panel rounded-[2rem] border border-white/80 p-8 text-center">
        <LoaderCircle className="h-8 w-8 animate-spin text-slate-400 mx-auto" />
  ...
  return (
    <div suppressHydrationWarning className="space-y-8">
      <section className="glass-panel rounded-[2rem] border border-white/80 p-6 sm:p-8">
        ...
        {message ? (
          <div className={`mt-6 flex items-start gap-3 rounded-2xl px-4 py-3 text-sm ${message.tone === "error" ? "border border-rose-100 bg-rose-50 text-rose-700" : "border border-emerald-100 bg-emerald-50 text-emerald-700"}`}>
