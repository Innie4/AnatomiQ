        ...
        {topics.map((topic, index) => (
          <FadeIn key={topic.id} delay={index * 0.05} className="glass-panel rounded-[1.75rem] border border-white/80 p-6">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {topic.childTopics.map((child) => (
                <div key={child.id} className="rounded-2xl border border-slate-200 bg-white/90 p-4">
                  ...
                  <Link href={`/exam?${activeCourse ? `course=${activeCourse}&` : ""}topic=${topic.slug}&subtopic=${child.slug}`} className="mt-3 inline-flex text-sm font-semibold text-sky-700">
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <Link
                href={`/exam?${activeCourse ? `course=${activeCourse}&` : ""}topic=${topic.slug}`}
